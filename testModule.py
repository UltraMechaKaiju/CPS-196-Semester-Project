...
class playerCharacter(pygame.sprite.Sprite):
    def __init__(self, startLocation: pygame.Vector2, *groups):
      ...
      self.hitbox = pygame.Rect((0,0),(100,50))
      self.velocity = 0.0
...
def moveWorldOffset(moveVec, targetCharacter: playerCharacter, currentScene: Scene):
    ...
    characterOffset = pygame.Vector2(0,0)
    ...
    if targetCharacter.PPos.y <= targetCharacter.hitbox.width/2:
        targetCharacter.PPos.y = targetCharacter.hitbox.width/2
    if targetCharacter.PPos.x <= targetCharacter.hitbox.height/2:
        targetCharacter.PPos.x = targetCharacter.hitbox.height/2
    #Check for collisions with the right and bottom of the world
    if currentScene.worldExtents.x <= targetCharacter.PPos.x + targetCharacter.hitbox.height/2:
        targetCharacter.PPos.x = currentScene.worldExtents.x - targetCharacter.hitbox.height/2
    if currentScene.worldExtents.y <= targetCharacter.PPos.y + targetCharacter.hitbox.width/2:
        targetCharacter.PPos.y = currentScene.worldExtents.y - targetCharacter.hitbox.width/2
    ...
    if targetCharacter.PPos.y <= currentScene.cameraExtents.y/2:
        ...
        characterOffset.y = currentScene.cameraExtents.y/2
        cameraOffset.y = targetCharacter.PPos.y
    ...
    if targetCharacter.PPos.x <= currentScene.cameraExtents.x/2:
        ...
        characterOffset.x = currentScene.cameraExtents.x/2
        cameraOffset.x = targetCharacter.PPos.x
    ...
    #set equal to worldOffset and characterOffset
...
testScene = Scene(pygame.Vector2(1920,1080), pygame.Vector2(250,250), pygame.Vector2(0,0), worldObject)
...
while True:
    ...
    if (desiredMove != pygame.Vector2(0,0)):
        desiredMove = desiredMove.normalize() * 50
    ...
    screen.blit(characterSprite.colorBlock, testScene.characterOffset)
